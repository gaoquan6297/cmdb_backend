        ...
        if request.data['mode'] == 'put':
            ...
            if not query_switch['query']:
                ...
                userinfo = Account.objects.filter(
                    username=audit, group__in=['admin','perform']).first()
                try:
                    thread = threading.Thread(
                        target=push_message,
                        args=(
                            {'to_user': request.user, 'workid': work_id, 'cname':connection_name}, 5, request.user, userinfo.email, work_id,
                            '提交'))
                    thread.start()
                except Exception as e:
                    CUSTOM_ERROR.error(f'{e.__class__.__name__}: {e}')
