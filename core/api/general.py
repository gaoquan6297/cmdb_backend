    ...
    def put(self, request, args=None):
        ...
        if args == 'connection':
            try:
                ...
                print("user:",request.data['permissions_type'])
                ...
                info = Account.objects.filter(group__in=['admin','perform']).all()
                serializers = UserINFO(info, many=True)
                # print(con_name, serializers.data,assigned['person'],custom_com['con_room'],custom_com['multi'] )
                return Response(
                    {
                        'connection': con_name,
